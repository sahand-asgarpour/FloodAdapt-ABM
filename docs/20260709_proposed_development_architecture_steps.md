...
| **P0** | PRE.1 — pin honeybees/mesa in isolated env; import/instantiate test | 2026-07-09 review | 4b-pre | **PREPARED** — kit in `verification/preflight_4b_full/`; execute on dev machine, then pin |
| **P0** | PRE.3 — fix `engine.reset_state()` shared-engine wart in `FloodAdaptSLRModel` | 2026-07-09 review (`mesa_native.py:174`) | 4b-pre | **DONE** (`state_epoch` guard + 4 tests, commit `7ceb144`) |
| **P0** | HYG.1 — rewrite stale root `README.md` | 2026-07-09 review | 4b-pre | **DONE** (commit `d05a97a`) |
| **P1** | PRE.2 — run gates on real Charleston table; profile `engine.step()` | 2026-07-09 review | 4b-pre | **PREPARED** — runner in `verification/real_table_gate/`; execute on dev machine |
| **P1** | VER.1 — re-run V1–V6 battery on `SimulationEngine`+`SEURule`; regenerate V5 (lifespan now closed) | 2026-07-09 review | 4b-pre | OPEN |
...
| **P2** | VER.2 — verification bundle portability (figures/, data/, env-var paths, README rename) | 2026-07-09 review | 4b-pre | **DONE** except figures (regenerate via VER.1) |
...
| 1 | Validated SEU | §12.1 battery passes; SEU == DYNAMO-M within tol. | ✅ PASS (on bridge; re-run on engine = VER.1) |
...
| **4b-pre** | **De-risking + hygiene (this doc)** | **PRE.1–3 + HYG.1–3 + VER.1 complete; CI green; README/AGENTS accurate; battery re-run on engine** | 🟡 IN PROGRESS (2026-07-09: PRE.3/4 + HYG.1–4 applied; PRE.1/2 prepared awaiting execution; VER.1 + first pytest/CI run open — see `20260709_performed_tasks.md`) |
...
**Gate rule:** do not start 4b-full step 2 until the 4b-pre P0 items (PRE.1, PRE.3, HYG.1) and P1 items (PRE.2, VER.1, HYG.2, HYG.3) are closed. PRE.4 may run in parallel with 4b-full step 2.
